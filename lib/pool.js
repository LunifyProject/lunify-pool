...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage)
{
 // function handleMinerMethod(req, res){
    ...
    switch(method){
        ...
        if(!PoolRedirect.isEnabled() || (miner.agent.name.toLowerCase() !== 'lfirig') || (miner.agent.versionInt < 5202)) {
        	noRedirectAction();
		break;
	}
 ...
 **/
 function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    const rewardType = miner.poolType
   ...
   redisCommands = paymentSystem(poolType).recordShare(redisCommands, miner, job, shareDiff, hashHex, shareType, blockTemplate);
    ...
    redisCommands.push(['zadd', coin + ':hashrate', dateNowSeconds, [diff, login, dateNow, rewardType].join(':')])
    ...
    if (workerAlias) {
        redisCommands.push(['zadd', coin + ':hashrate', dateNowSeconds, [diff, workerAlias, dateNow, rewardType].join(':')]);
